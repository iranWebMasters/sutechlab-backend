...
class HomeView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, self.template_name, {"home": home_page, "banners": banners}
        )
...
class AboutView(View):
    ...
    def get(self, request):
        about_us_data = (
            AboutUs.objects.all()
        )  # Fetch the first AboutUs object, adjust as needed
        context = {
            "about": about_us_data,
        }
...
class ContactUsView(FormView):
    ...
    def form_invalid(self, form):
        messages.error(
            self.request,
            "در پردازش درخواست شما خطایی رخ داده است. لطفاً دوباره امتحان کنید",
        )
