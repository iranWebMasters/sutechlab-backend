...
class OrderCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_success_url(self):
        return reverse_lazy(
            "orders:sample_info_create", kwargs={"order_code": self.object.order_code}
        )
    ...
    def get_redirect_url(self):
        return reverse(
            "orders:sample_info_create", kwargs={"order_code": self.order.order_code}
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        incomplete_order = Order.objects.filter(
            user=self.request.user, is_complete=False, experiment=experiment
        ).first()
        context.update(
            {
...
class OrderUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self, queryset=None):
        ...
        return get_object_or_404(
            Order, user=self.request.user, is_complete=False, experiment=experiment
        )
...
class SampleDeleteView(DeleteView):
    ...
    def get_success_url(self):
        order_code = self.kwargs.get("order_code")
        return reverse_lazy(
            "orders:sample_info_create", kwargs={"order_code": order_code}
        )
...
class TestInfoCreateView(FormView):
    ...
    def get_redirect_url(self):
        return reverse(
            "orders:test_info_create", kwargs={"order_code": self.order.order_code}
        )
    ...
    def form_invalid(self, form):
        messages.error(
            self.request, "لظفا تمام موارد خواسته شده را به درستی پر نمایید."
        )
        for field, errors in form.errors.items():
            for error in errors:
                messages.error(self.request, f"{field}: {error}")
...
class TestParametersView(View):
    def get(self, request, test_id, *args, **kwargs):
        try:
            ...
            parameters_data = [
                {"id": param.id, "name": param.name} for param in parameters
            ]
...
class TestUpdateView(UpdateView):
    ...
    def get_success_url(self):
        experiment_id = self.kwargs["experiment_id"]
        return reverse_lazy(
            "orders:test_info_create", kwargs={"experiment_id": experiment_id}
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        user_samples = SampleInfo.objects.filter(
            experiment=experiment, user=current_user
        )
        user_test = get_object_or_404(
            TestInfo, experiment=experiment, user=current_user, pk=test_pk
        )
        tests = experiment.tests.all()
        context.update(
            {
...
class DiscountInfoFormView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if DiscountInfo.objects.filter(order=self.order).exists():
            messages.error(
                request,
                "اطلاعات تخفیف برای این سفارش قبلاً ثبت شده است. لطفاً سفارش جدید ثبت کنید.",
            )
    ...
    def get_redirect_url(self):
        return reverse(
            "orders:discount_info", kwargs={"order_code": self.order.order_code}
        )
...
class UserOrderCancelView(View):
    ...
    def get(self, request, order_code):
        ...
        return render(request, self.template_name, {"order": order, "referer": referer})
