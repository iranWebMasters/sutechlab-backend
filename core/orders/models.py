...
class TemporaryRequestInfo(models.Model):
    ...
    def __str__(self):
        return f"TemporaryRequestInfo(ID: {self.id}, User: {self.user.email}, Experiment: {self.experiment.test_name}, Date: {self.submission_date}, Order Code: {self.order_code})"

<<<<<<< HEAD
class TemporarySampleInfo(models.Model):
=======
    class Meta:
...
class TemporaryTestInfo(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name='کاربر')
    experiment = models.ForeignKey(Experiment, on_delete=models.CASCADE, verbose_name='آزمایش')
    user_sample = models.ForeignKey(TemporarySampleInfo, on_delete=models.CASCADE, verbose_name='شناسه نمونه آزمایش')
...
class TemporaryDiscountInfo(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, verbose_name='کاربر')
    experiment = models.ForeignKey(Experiment, on_delete=models.CASCADE, verbose_name='آزمایش')
...
class TemporaryOrder(models.Model):
    ...
    request_info = models.OneToOneField('TemporaryRequestInfo', on_delete=models.CASCADE, null=True, blank=True, verbose_name='اطلاعات درخواست')
    sample_info = models.ManyToManyField('TemporarySampleInfo', related_name='requests', verbose_name='اطلاعات نمونه')
    test_info = models.ManyToManyField('TemporaryTestInfo', verbose_name='اطلاعات آزمایش')
    discount_info = models.OneToOneField('TemporaryDiscountInfo', on_delete=models.CASCADE, null=True, blank=True, verbose_name='اطلاعات تخفیف')
    is_complete = models.BooleanField(default=False, verbose_name='تکمیل شده')

    def __str__(self):
        return f"TemporaryOrder(ID: {self.id}, User: {self.user.email}, Experiment: {self.experiment.test_name})"
