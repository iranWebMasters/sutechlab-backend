...
class Order(models.Model):
    ...
    experiment = models.ForeignKey(
        Experiment, on_delete=models.CASCADE, verbose_name="آزمایش"
    )
    ...
    invoice_pdf = models.FileField(
        upload_to="invoices/", null=True, blank=True, verbose_name="پیش فاکتور"
    )
    order_code = models.CharField(
        max_length=255, blank=True, unique=True, verbose_name="کد سفارش"
    )
    ...
    tracking_code = models.CharField(
        max_length=255, null=True, blank=True, verbose_name="Tracking Code"
    )
...
class SampleInfo(models.Model):
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, related_name="sample_info"
    )
    sample_type = models.CharField(max_length=50, verbose_name="نوع نمونه")
    customer_sample_name = models.CharField(
        max_length=255, verbose_name="نام نمونه مشتری"
    )
...
class TestInfo(models.Model):
    ...
    user_sample = models.ForeignKey(
        SampleInfo, on_delete=models.CASCADE, verbose_name="شناسه نمونه آزمایش"
    )
    ...
    parameter = models.ForeignKey(
        Parameters, on_delete=models.CASCADE, verbose_name="پارامتر"
    )
    ...
    def get_parameter_values_dict(self):
        try:
            ...
            formatted_output = ", ".join(
                f"{key} : {value}" for key, value in parameter_dict.items()
            )
...
class DiscountInfo(models.Model):
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, related_name="discount_info"
    )

    send_cost = models.BooleanField(
    ...
    is_affiliated_with_institution = models.BooleanField(
        default=False,
        verbose_name="آیا کاربر متقاضی استفاده از تخفیف نهادهای طرف قرارداد است؟",
    )
    contract_party_file = models.FileField(
