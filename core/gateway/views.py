...
class GoToGatewayView(View):
    def get(self, request, payment_id):
        ...
        print(
            f"GoToGatewayView: {request.user} (is_authenticated: {request.user.is_authenticated})"
        )

        user_mobile_number = (
            request.user.profile.phone_number if request.user.is_authenticated else None
        )
        ...
        try:
            ...
            messages.error(
                request,
                "اتصال به درگاه پرداخت ناموفق بود ، لطفا اتصال خود را به اینترنت بررسی نمایید و دوباره امتحان کنید",
            )
...
class CallbackGatewayView(View):
    def get(self, request, payment_id):  # Add payment_id as a parameter
        print(
            f"User in callback: {request.user} (is_authenticated: {request.user.is_authenticated})"
        )
        ...
        if bank_record.is_success:
            ...
            return redirect(
                "userpanel:payment_success", tracking_code=tracking_code
            ...
            messages.error(
                request,
                "پرداخت با شکست مواجه شده است. اگر پول از حساب کم شده است ظرف مدت ۴۸ ساعت پول به حساب شما بازخواهد گشت.",
            )
